...
test('resolveDisplayRoles: fake returns brand + composed product → brand: "Homeo-Vita", product: "MULTIVITAMIN GUMMIES", confidence: 0.85', async () => {
  const fakeClient = {
    askJson: async () => ({
      brand: 'Homeo-Vita',
      productName: 'Gummies Multivitamin',
      confidence: 0.9
    })
  };
...
test('buildDisplayRolePrompt: returns correct prompt template', () => {
