...
class ComparisonService:
    ...
    def compare_lists(self, list_a: List[str], list_b: List[str]) -> str:
        ...
        # Compare claims/ingredients by aggregating semantic similarity
        # A simple approach: average the best match for each item in list_a
        total_score = 0
        for item_a in list_a:
            emb_a = self.encoder.encode(item_a, convert_to_tensor=True)
            emb_b = self.encoder.encode(list_b, convert_to_tensor=True)
            cosine_scores = util.cos_sim(emb_a, emb_b)
            total_score += torch.max(cosine_scores).item()
            
        avg_score = total_score / len(list_a)
    ...
    def evaluate_labels(self, source: ExtractedLabel, target: ExtractedLabel) -> Dict[str, str]:
        ...
        return {
            ...
            "colour_theme": self.compare_exact_or_missing(source.colour_theme, target.colour_theme),
            ...
            "logo": self.compare_exact_or_missing(source.logo, target.logo),
    ...
    def find_best_cross_company_match(self, source_label: ExtractedLabel, external_labels: List[ExtractedLabel]) -> Optional[Dict]:
        ...
        for ext_label in external_labels:
            ...
            # Calculate a simple match score (Matches = 2, Similar = 1, Conflict = 0)
            score = 0
            for status in eval_results.values():
