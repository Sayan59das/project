...
**A real quality finding from continuing validation after PR #16 opened** (5 more of the 14 checked, same session, before a hard time cutoff): `MV VF IRN118-2.pdf` gained a clean, correct-looking 15-item multivitamin declaration -- a second real win. But `LXIR Shilajit STICK VF IRN19-1.pdf` recovered 2 items that are wrong: "Vrishya (Aphrodisiac)" and "Vajikar (Enhances Vigor and Vitality)" -- real, printed text on the label (so they legitimately passed whole-phrase corroboration), but Ayurvedic PROPERTY/benefit descriptions, not ingredients. The no-anchor locator found a real run of small print, just the wrong panel -- a benefits section, not the ingredients declaration. This is not fabrication (nothing invented; score.py's fabrication table wouldn't catch it either, since the label likely does have real ingredients elsewhere) -- it would show up as WRONG, the same risk category every other source in this pipeline already carries. Three other labels in this batch stayed safely empty (no false content). Not fixed tonight, for time -- named as the clear next thing to tighten (e.g. requiring the run's own text to have comma-list shape, or excluding known Ayurvedic-property phrasing, before trusting a no-anchor match) before broader real-data trust or a `score.py` re-measurement.

**The LXIR Shilajit STICK wrong-panel bug is now fixed**: `findIngredientsPanelFromLines` requires the winning run's own text to have comma-list shape (>=2 commas across its lines -- the same "two ingredients is the floor" convention `MIN_ITEMS` and `extractIngredients`'s own comma check already apply, a generic structural signal, not a specific word or brand). A real ingredients declaration is comma-separated; a benefits/properties section of short standalone phrases ("Vrishya (Aphrodisiac)", "Vajikar (Enhances Vigor and Vitality)") is not, and now correctly fails to qualify as a candidate run. New regression test pins the exact bug. 22/22 tests pass; full suite still clean (631/620, same pre-existing failures). The remaining 4 of 14 labels are still unvalidated by direct inspection, but the specific failure mode found is now closed by a generic, tested rule rather than a one-off patch.

**A second by-source gating attempt, tried and REVERTED after a real regression** (same session, same discipline as the `tesseract-region-ocr` attempt below): the by-source table also shows brand_name/product_name from Tesseract's full-page-OCR family net-negative in aggregate (brand_name 1 correct/17 wrong; product_name 0/7 and 0/5 across its two sub-passes). Unconditionally blanking both so only VLM role resolution could fill them broke the regression oracle on 6 of 14 fixtures -- both synthetic (a clean OCR-only JPG whose brand "VitaFit" this source reads correctly and reliably) and real (Apple Cider Vinegar Gummy, Chyawanprash Gummies, She-Arise Gummies, a real-world JPG pattern). Exactly the Step 5 lesson repeating: the aggregate signal hides that this source is reliable on clean, well-formed artwork and unreliable on this client's messier real intake; a blanket gate fixes the second group by breaking the first. Reverted immediately on discovering the regression oracle failure, kept only as a code comment recording the attempt. A real fix would need VLM role resolution to actively outvote a Tesseract guess when both are available, not Tesseract blanked pre-emptively regardless of whether a better answer ever arrives -- left as a named, unresolved follow-up.
