...
**Not fully validated**: 4 of the 14 labels still unchecked (`PMS VF IRN71-1.pdf`, `Shilajit Honey Sticks (16).pdf`, `Wellmedic VF Chawan IRN25-2.pdf`, `Wellmedic VF Relax IRN27-2.pdf`), for time (this update lands right at a hard session cutoff). Given the LXIR Shilajit STICK finding above, **do not merge PR #16 without either fixing the wrong-panel risk or completing this validation pass first** -- the mechanism's failure mode is "wrong," not "fabricated," but "wrong" still needs to be rare enough to trust, and one bad case in nine checked so far is not yet that.
