// Shared artwork file preview — image inline, a real rendered thumbnail for
...
// embedded via <iframe> with the browser's native PDF viewer — that takes
...
// Only these three fields are ever read below — widened from `Artwork` (via
...
function NoPreview({ maxHeight, iconSize, caption }: { maxHeight: number; iconSize: number; caption?: string }) {
  return (
    <Box sx={{ height: maxHeight, display: 'grid', placeItems: 'center', textAlign: 'center', px: 2 }}>
      <Box>
        <MdInsertDriveFile size={iconSize} color="#9EA4AB" />
        <Typography variant="caption" sx={{ display: 'block', color: '#9EA4AB', mt: 1 }}>
  ...
  if (loading) {
    return (
      <Box sx={{ height: maxHeight, display: 'grid', placeItems: 'center' }}>
        <CircularProgress size={24} sx={{ color: '#E26737' }} />
      </Box>
    );
  }
  if (error || !url) {
...
export function ArtworkPreview({ artwork, variant = 'dashed', height = 200 }: Props) {
  if (variant === 'card') {
    if (!artwork) {
      return (
        <Box sx={{ height, borderRadius: 2, bgcolor: '#EEF1F4', display: 'grid', placeItems: 'center' }}>
    ...
    return (
      <Box sx={{ height, borderRadius: 2, overflow: 'hidden', bgcolor: '#EEF1F4' }}>
  ...
  if (variant === 'panel') {
    return (
      <Box sx={{ borderRadius: 3, overflow: 'hidden', bgcolor: '#EEF1F4', minHeight: 220 }}>
        {artwork?.filePath && artwork.fileType.startsWith('image/') ? (
  ...
  // 'dashed' — QAPage's original look, also the default for new callers.
  if (!artwork) {
    return (
      <Box
        sx={{
  ...
  if (artwork.filePath && artwork.fileType.startsWith('image/')) {
    return (
      <Box sx={{ border: '1px dashed #D8DDE3', borderRadius: 3, minHeight: 220, display: 'grid', placeItems: 'center', overflow: 'hidden' }}>
  ...
  if (artwork.filePath && artwork.fileType === 'application/pdf') {
    return (
      <Box sx={{ border: '1px dashed #D8DDE3', borderRadius: 3, overflow: 'hidden' }}>
